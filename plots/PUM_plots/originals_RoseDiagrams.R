...
# This code contains three different plots:
#   - Rose Diagrams from april 1855 to march 1856 (Nighting_Plot1)
#   - Rose Diagrams from april 1854 to march 1855 (Nighting_Plot2)
#   - Both Rose Diagrams (Nighting_Plot1 and Nighting_Plot2) with the explanatory text from FN
...
Nighting_Plot1 <- melt(Nighting_df,measure.vars=c("bar1", "bar2", "bar3"), variable.name = "bar") %>%
  ...
  labs(subtitle = "APRIL 1854 TO  MARCH 1855")+ 
...
Nighting_Plot2 <- melt(Nighting_df2,measure.vars=c("bar1", "bar2", "bar3"), variable.name = "bar") %>%
  ...
  theme_void()+ 
  labs(subtitle = " APRIL 1855 TO  MARCH 1856")+
  theme(legend.position = "none",
        plot.subtitle = element_markdown(color = "black", hjust = 0.5, lineheight = 1.5, family="serif",margin = margin(t=0,b = -50), size=14),
        plot.margin = margin(b=-100, t=5))+
...
cowplot::plot_grid(Nighting_Plot1,Nighting_Plot2)

##############################################################################
# with explanation as FN did:
# (this only works well with mac)

# modifying sizes and typography
Nighting_Plot1 <- melt(Nighting_df,measure.vars=c("bar1", "bar2", "bar3"), variable.name = "bar") %>%
  ggplot(aes(x=month(Date), y=value))+
  geom_bar(aes(fill=factor(bar, levels=c("bar3", "bar2", "bar1")),color=factor(bar, levels=c("bar3", "bar2", "bar1"))),stat="identity",width = 1, alpha=0.7)+
  coord_polar(start=radiansStart, clip = "off")+
...
title_words <- ggplot(data.frame(x = 1:2, y = 1:10)) +
  ...
  theme(line = element_blank(),
        axis.text = element_blank(),
        axis.line.x = element_blank(),
        # plot.title = element_markdown(size = 16, color = "black", family="Trattatello", hjust = 0.5), #commented for windows' version
        plot.title = element_markdown(size = 16, color = "black",   hjust = 0.5), #just for windows' version
